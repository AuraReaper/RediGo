...
func evalSET(args []string) []byte {
	...
	for i := 2; i < len(args); i++ {
		...
		case "EX", "ex":
			...
			exDurationMs, err := strconv.ParseInt(args[3], 10, 64)
			if err != nil {
				return Encode(errors.New("ERR value is not an integer or out of range"), false)
			}
			exDurationMs *= 1000
		default:
			return Encode(errors.New("ERR synatx error"), false)
		}
	}
...
func executeCommand(cmd *RedigoCmd, c *Client) []byte {
	switch cmd.Cmd {
	case "PING":
...
func executeCommandToBuffer(cmd *RedigoCmd, buf *bytes.Buffer, c *Client) {
	buf.Write(executeCommand(cmd, c))
}

func EvalAndRespond(cmds RedigoCmds, c *Client) {
	var response []byte
